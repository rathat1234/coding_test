...
void pool_init(MemoryPool *pool)
{
    ...
    {
        Block *current = pool->memory_area + (i * BLOCK_SIZE);
        Block *next = pool->memory_area + ((i + 1) * BLOCK_SIZE);
        current->next = next;
        if (i == 0)
...
void *pool_alloc(MemoryPool *pool)
{
    ...
    void *temp = pool->free_list;
    Block *next = pool->free_list->next;
    pool->free_list = next;
    pool->used_count++;
    return temp;
}
...
// 테스트 코드
int main()list의 head가 됨

    // 다시 1개 할당 (p2와 같은 주소가 나와야 함 -
{
    ...
    printf("Allocated: 0x%p, 0x%p, 0x%p\n", p1, p2, p3);
    printf("Used Blocks: %d\n", my_pool.used_count);

    // 1개 해제 (p2)
    printf("Freeing 0x%p...\n", p2);
    pool_free(&my_pool, p2); // 이제 p2가 free_ LIFO 특성)
    void *p4 = pool_alloc(&my_pool);
    printf("Re-allocated: 0x%p (Should be same as old p2)\n", p4);
